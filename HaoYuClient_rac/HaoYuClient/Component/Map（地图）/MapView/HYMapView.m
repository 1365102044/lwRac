...
@interface HYMapView ()<XJClusterAnnotationViewDelegate>
{
    ...
    BOOL _isDisplayMyLoction;//是否显示自己指定的坐标
...
/**
 设置地图找房的数据 以及 地图中心坐标
 */
- (void)setHouseDatas:(NSArray *)houseDatas CenterCoordinate:(CLLocationCoordinate2D)CenterCoordinat
{
    _HousesDatas = houseDatas;
    [self updateModelDatas];
    [_myMapView setCenterCoordinate:CenterCoordinat animated:YES];
}
...
- (void)addPointViewWithlat:(double)lat lng:(double)lng
{
    ...
    LWLog(@"\n\n%@++++++++%d+++++++++%d",_myMapView,coor.latitude,coor.longitude);
}
...
- (void)mapView:(BMKMapView *)mapView didSelectAnnotationView:(BMKAnnotationView *)view
{
    ...
    if ([self.delegate respondsToSelector:@selector(mapView:didSelectAnnotationView:dataModel:)]) {
        [self.delegate mapView:mapView didSelectAnnotationView:view dataModel:clusterAnnotation.cluster.HouseInfomodel];
    }
}
...
- (void)mapView:(BMKMapView *)mapView onClickedMapBlank:(CLLocationCoordinate2D)coordinate
{
    if ([self.delegate respondsToSelector:@selector(mapView:onClickedMapBlank:)]) {
        [self.delegate mapView:mapView onClickedMapBlank:coordinate];
    }
}
...
- (void)mapViewDidFinishLoading:(BMKMapView *)mapView {
    ...
    if (_isDisplayMyLoction) {
        [_myMapView addAnnotation:myAnnotation];
        [_myMapView setCenterCoordinate:myAnnotation.coordinate animated:YES];
        [self setMapRegion:0.1];
    }
    LWLog(@"\n\n++++++++++地图加载完毕++++")
}
