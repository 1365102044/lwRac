...
- (void)LoginOut
{
    ...
    USERDEFAULTS_RE(USER_INFOR_PASSWORD_KEY);
    ...
    //清空本地数据库
    USERDEFAULTS_RE(SAVE_DB_MESSAGE_LAST_UPDATE_DATE);
    [[LWFmdb share] deleteTableWithTableType:LWDBTableTypeMessge];
}
...
- (void)saveUser_Info:(id)info
{
    if ([info isKindOfClass:[NSDictionary class]]) {
        ...
        if (info[@"userPhoneNumber"]) USERDEFAULTS_SET([HYStringTool checkString:info[@"userPhoneNumber"]], USER_INFOR_PHONE);
        if (info[@"phone"]) USERDEFAULTS_SET([HYStringTool checkString:info[@"phone"]], USER_INFOR_PHONE);
        if (info[@"id"]) USERDEFAULTS_SET([HYStringTool checkString:info[@"id"]], USER_INFOR_USERID);
        if (info[@"userId"]) USERDEFAULTS_SET([HYStringTool checkString:info[@"userId"]], USER_INFOR_USERID);
...
- (void)saveUser_InforOfUserInfo:(id)info
{
    ...
    if ([info isKindOfClass:[NSDictionary class]]) {
        ...
        USERDEFAULTS_SET([HYStringTool checkString:info[@"password"]], USER_INFOR_PASSWORD_KEY);
    }
    [SYSTEM_USERDEFAULTS synchronize];
...
//    [[XGPushTokenManager defaultTokenManager] clearAllIdentifiers:XGPushTokenBindTypeTag];
    ...
    if (USERDEFAULTS_GET(USER_INFOR_USERID)){
        [[XGPushTokenManager defaultTokenManager] unbindWithIdentifer:USERDEFAULTS_GET(USER_INFOR_USERID) type:XGPushTokenBindTypeAccount];
    }
//    [[XGPushTokenManager defaultTokenManager] bindWithIdentifier:@"" type:XGPushTokenBindTypeAccount];
    ...
    NSArray *tags = [[XGPushTokenManager defaultTokenManager] identifiersWithType:XGPushTokenBindTypeTag];
    NSArray *accounts = [[XGPushTokenManager defaultTokenManager] identifiersWithType:XGPushTokenBindTypeAccount];
