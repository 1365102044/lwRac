...
- (void)setValueUserInfor
{
    if ([HYUserInfor_LocalData share].User_HeaderImageData_Local) {
        ...
        [[HYUserInfor_LocalData share] saveUser_Headericon_Local:_headericon.image];
    }
...
- (void)toMineInforVC
{
    ...
    [[self currentViewController].navigationController pushViewController:inforVC animated:YES];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        ADD_NOTI(updateMsgIcon, GETMESSAGEINFORKNOEISORNOTNOLOOKMESSAGE_FOR_PUSH);
...
- (HYBaseView*)inforView
{
    if (!_inforView) {
        ...
        HYDefaultLabel *nicklabe = [HYDefaultLabel labelWithFont:SYSTEM_MEDIUMFONT(22) text:@"" textColor:HYCOLOR.color_c4];
        nicklabe.backgroundColor = [UIColor clearColor];
        HYDefaultLabel *qianminglabe = [HYDefaultLabel labelWithFont:SYSTEM_MEDIUMFONT(13) text:@"" textColor:HYCOLOR.color_c4];
        qianminglabe.backgroundColor = [UIColor clearColor];
        [headericon setBoundWidth:0 cornerRadius:ADJUST_PERCENT_FLOAT(35)];
        _msgBtn = megsbtn;
        ...
        [headericon mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_offset(CGSizeMake(ADJUST_PERCENT_FLOAT(70), ADJUST_PERCENT_FLOAT(70)));
        ...
        [megsbtn bk_whenTapped:^{
            [HYMessageViewController pushMessageViewControllerWithSourceVC:[self currentViewController]];
        }];
