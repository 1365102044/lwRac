...
//费用类型
#define FEIYONGTYPEID       @"933283b8-3447-4582-b893-9ac266f387ce"
//渠道来源 channelSourceId
#define CHANNELSOURCEID     @"4bed679d23b942e4bd4de4ad28b7a138"
//成交类型id chengjiaoTypeId
#define  CHENGJIAOTYPEID     @"63007bd34b544389b3897ee158a8f2b7"
              ...
              callBackBlock:(HYEventCallBackBlock)callBackBlock
{
    ...
    PARA_CREART;
    PARA_SET(cityId, @"cityId");
    [[HYServiceManager share] postRequestAnWithurl:GET_PROJECT_LIST_INFOR_URL
                                       paramters:para
                                    successBlock:^(id objc, id requestInfo) {
                                        ...
                                        for (NSDictionary *dict in tem_a) {
                                            ...
                                            if ([model.itemStatus integerValue] != 4) {                                            
                 ...
                 callBackBlock:(HYEventCallBackBlock)callBackBlock
{
    ...
    [[HYServiceManager share] postRequestAnWithurl:GET_HUXING_LIST_INFOR_URL
                                       paramters:para
                                    successBlock:^(id objc, id requestInfo) {
                                        LWLog(@"\n意向户型列表：%@",objc);
                                        [self.DatasMutableArray removeAllObjects];
                       ...
                       callBackBlock:(HYEventCallBackBlock)callBackBlock
{
    ...
    [[HYServiceManager share] postRequestAnWithurl:GET_FANGJIANNUMBER_LIST_INFOR_URL
                                       paramters:para
                                    successBlock:^(id objc, id requestInfo) {
                                        LWLog(@"\n房间号列表：%@",objc);
                                        [self.DatasMutableArray removeAllObjects];
...
- (void)requestProjectInforByCityForMapFindHouseCallBackBlock:(HYEventCallBackBlock)callBackBlock
{
    ...
    [[HYServiceManager share] postRequestAnWithurl:GET_PROJECT_LIST_BYCITY_INFOR_URL
                                       paramters:para
                                    successBlock:^(id objc, id requestInfo) {
...
- (void)ZuKeQianYuCreateBill:(NSDictionary *)param callBackBlock:(HYEventCallBackBlock)callBackBlock
{
    ...
    para[@"feiYongTypeId"] = FEIYONGTYPEID;
    [para removeObjectForKey:@"shouZhiList"];
    PARA_SET(@"1", @"type");
    PARA_SET(@"1", @"isJizhong");
    PARA_SET(@"1", @"signType");
    [[HYServiceManager share] postRequestAnWithurl:COMMIT_QIANYUEHOUSTE_INFOR_URL
                                      ...
                                      successBlock:^(id objc, id requestInfo) {
                                          ...
                                          NSMutableArray *bill_M_Arr = [[NSMutableArray alloc] init];
                                          for (NSDictionary *dic in billArr) {
                                              NSMutableDictionary *dic_M = [[NSMutableDictionary alloc] init];
                                              dic_M[@"typeId"] = dic[@"feiYongTypeId"];
                                              dic_M[@"isIncludeDJ"] = @"0";
                                              dic_M[@"beginTime"] = dic[@"bqBeginDate"];
                                              dic_M[@"desc"] = dic[@"feiYongDesc"];
                                              dic_M[@"endTime"] = dic[@"bqEndDate"];
                                              dic_M[@"money"] = dic[@"bqMonthMoney"];
                                              dic_M[@"predictTime"] = dic[@"bqBackPayDate"];
                                              dic_M[@"type"] = dic[@"type"];
                                              dic_M[@"discountedPrice"] = dic[@"discountedPrice"];
                                              dic_M[@"originalAmount"] = dic[@"originalAmount"];//原单金额
                                              [bill_M_Arr addObject:dic_M];
                                          }
                                          if (self.callBackBlock) {
                                              self.callBackBlock(bill_M_Arr);
                                          }
                                      } failureBlock:^(id error, id requestInfo) {
...
- (void)QianYueCreateHeTong:(NSDictionary *)param callBackBlock:(HYEventCallBackBlock)callBackBlock
{
    ...
    PARA_CREART;
    [para addEntriesFromDictionary:param];
    para[@"feiYongTypeId"] = FEIYONGTYPEID;//费用类型
    para[@"channelSourceId"] = CHANNELSOURCEID;//渠道来源
    para[@"certificateType"] = @"1";////证件类型：1.身份证，2.护照，3.港澳通行证，4.台湾同胞证
    para[@"chengjiaoTypeId"] = CHENGJIAOTYPEID;//成交类型id
...
//    para[@"electronicTemplateId"] = @"";//电子合同模版id  ++
    ...
    para[@"startTime"] = param[@"beginDate"];//转换字段
    para[@"endTime"] = param[@"endDate"];//转换字段
