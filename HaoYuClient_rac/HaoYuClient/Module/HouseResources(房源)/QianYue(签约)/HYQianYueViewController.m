...
//同意协议的视图
@property (nonatomic, strong) HYBaseView * agreeXiYiView;
//视图同意协议
@property (nonatomic, assign) BOOL  agreeBool;
@property (nonatomic, strong) UIButton * xiyi_icon_btn;
    ...
    [[AipOcrServiceManger share] generalBasicOCR:image
                                     ResultBlock:^(id sender) {
                                         dispatch_async_on_main_queue(^{
                                             ...
                                             [AipOcrServiceManger share].idCardModel = nil;
                                         });
                                     } failBlock:^(id sender) {
...
{
    _xiyi_icon_btn.selected = !_xiyi_icon_btn.selected;
    self.agreeBool = _xiyi_icon_btn.selected;
}
- (void)clickBottomBtn:(UIButton *)sender
{
    if ([sender.titleLabel.text isEqualToString:@"上一步"]) {
        if (_pageIndex == 1) {
            ...
            if (_pageIndex == 2) {
                self.agreeXiYiView.hidden = YES;
            }else if (_pageIndex == 3){
                self.agreeXiYiView.hidden = NO;
            }
            _pageIndex --;
        }
...
- (void)setUI
{
    ...
    [self.view addSubview:self.agreeXiYiView];
    ...
    [self.agreeXiYiView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.view.mas_left).mas_offset(MARGIN*2);
    ...
    [self.MainScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.self.bottomView.mas_top);
    }];
...
- (HYBaseView*)agreeXiYiView
{
    if (!_agreeXiYiView) {
        _agreeXiYiView = [[HYBaseView alloc] init];
        [_agreeXiYiView setBoundWidth:0 cornerRadius:6];
        
        HYDefaultButton *btn = [HYDefaultButton buttonImageAndTitleWithTitleStringKey:@"  勾选此项表示您认同以上签约协议"
                                                                           titleColor:HYCOLOR.color_c4
                                                                            titleFont:SYSTEM_REGULARFONT(13)
                                                                           imageNamed:@""
                                                                                 type:HYProjectButtonSetImage
                                                                               target:self
                                                                             selector:@selector(clickAgreeBtn:)];
        [_agreeXiYiView addSubview:btn];
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.mas_equalTo(_agreeXiYiView.mas_centerX).mas_offset(MARGIN*3);
            make.centerY.mas_equalTo(_agreeXiYiView.mas_centerY);
        }];
        UIButton *icon = [[UIButton alloc] init];
        [_agreeXiYiView addSubview:icon];
        [icon addTarget:self action:@selector(clickAgreeBtn:) forControlEvents:UIControlEventTouchUpInside];
        [icon setImage:IMAGENAME(@"mine_bill_select_n") forState:UIControlStateNormal];
        [icon setImage:IMAGENAME(@"mine_bill_select_s") forState:UIControlStateSelected];
        [icon mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_offset(CGSizeMake(MARGIN*2, MARGIN*2));
            make.right.mas_equalTo(btn.mas_left).mas_offset(5);
            make.centerY.mas_equalTo(btn.mas_centerY);
        }];
        _xiyi_icon_btn = icon;
        _agreeXiYiView.hidden = YES;
    }
    return _agreeXiYiView;
}
