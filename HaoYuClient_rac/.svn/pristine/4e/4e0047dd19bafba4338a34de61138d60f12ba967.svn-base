...
#define TOUSUJIANYILISTHAVEREPLYCELLIDNTIFIER @"TOUSUJIANYILISTHAVEREPLYCELLIDNTIFIER"
...
{
    HYTouSuJianYiModel *tousuModel = self.dataMutableArray[indexPath.row];
    LWTouSuJianYiBaseListTableViewCell *cell;
    
    if ([tousuModel.repairDetail isNotBlank] || tousuModel.resultPic.count > 0) {
        
        cell = [tableView dequeueReusableCellWithIdentifier:TOUSUJIANYILISTHAVEREPLYCELLIDNTIFIER];
        if(!cell){
            cell = [[LWTouSuJianYiReplyTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TOUSUJIANYILISTHAVEREPLYCELLIDNTIFIER];
        }
     
    }else{
        cell = [tableView dequeueReusableCellWithIdentifier:TOUSUJIANYILISTCELLIDNTIFIER];
        if(!cell){
...
- (void)setUI
{
    ...
    self.MainTableView.estimatedRowHeight = ADJUST_PERCENT_FLOAT(250);
