...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _coverL = [HYDefaultLabel labelWithFont:SYSTEM_REGULARFONT(10) text:@"" textColor:UIColor.whiteColor];
        [self addSubview:_coverL];
        _coverL.userInteractionEnabled = YES;
        _coverL.backgroundColor = [UIColor clearColor];
        ...
        [_coverL mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.top.bottom.mas_equalTo(self);
            make.width.mas_offset((SCREEN_WIDTH - MARGIN*2)/2);
        }];
