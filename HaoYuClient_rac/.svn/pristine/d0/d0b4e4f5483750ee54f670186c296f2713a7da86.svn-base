...
- (void)setBaoXiuModel:(HYBaoXiuModel *)BaoXiuModel
{
    ...
    BOOL replyViewHidden = NO;
    if ([BaoXiuModel.repairDetail isNotBlank] || BaoXiuModel.picurlArray.count > 0) {

        BOOL res = (BaoXiuModel.grade && ![BaoXiuModel.grade isEqualToString:@""]) ? YES: NO;
        [self.cellView.replyView setDataArray:BaoXiuModel.picurlArray replyContent:BaoXiuModel.repairDetail paramForPingJia:@{@"id":BaoXiuModel.customId?:@"",@"type":BaoXiuModel.modelType} hiddenPingJiaBtn:res];
    }else{
        replyViewHidden = YES;
    }
    [self.cellView hiddenReplyView:replyViewHidden];
}
