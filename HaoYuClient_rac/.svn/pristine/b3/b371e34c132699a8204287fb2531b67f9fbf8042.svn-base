...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.timeLable = [HYDefaultLabel labelWithFont:SYSTEM_MEDIUMFONT(14) text:@"2018-05-25" textColor:HYCOLOR.color_c4];
        self.storeLable = [HYDefaultLabel labelWithFont:SYSTEM_MEDIUMFONT(14) text:@"" textColor:HYCOLOR.color_c4];
...
- (void)confiItemsViewUI
{
    ...
    for (int i = 0; i< _itemsDataArr.count; i++) {
        ...
        itemview.priceLable.text = [NSString stringWithFormat:@"%@元",billModel.iosDedicated];
        ...
        if ([billModel.desc containsString:@"。"]) {
            ...
            if(_isShow){
                ...
                itemview.titleLable.text =  [NSString stringWithFormat:@"%@",billModel.typeMdodel.key];
            }
        }
//        NSLog(@"\n\n---------------%d-------------%lu",i,(unsigned long)self.itemsDataArr.count);
        if (i == 0) {
            [itemview mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.top.mas_equalTo(self.itemsView.mas_top).mas_offset(ADJUST_PERCENT_FLOAT(10));
                make.right.left.mas_equalTo(self.itemsView);
            ...
            [itemview mas_remakeConstraints:^(MASConstraintMaker *make) {
                ...
                if (i == _itemsDataArr.count - 1) {
                    make.bottom.mas_equalTo(self.itemsView.mas_bottom).mas_offset(-ADJUST_PERCENT_FLOAT(10)).priorityHigh();
                }
            }];
        }
    }
