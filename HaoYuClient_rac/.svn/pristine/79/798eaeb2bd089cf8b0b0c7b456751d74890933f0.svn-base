...
- (NSString *)ex_roundfWithScale:(NSInteger)scale
{
    ...
    // 四舍五入
    NSDecimalNumber *res = [[NSDecimalNumber decimalNumberWithString:self] decimalNumberByRoundingAccordingToBehavior:roundUp];
...
/**
 ...
 @return YES 是base64后的； NO 不是base64的
 */
- (BOOL)ex_isBase64String
{
    return [self ex_checkStringWithRegularExpression:@"^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{4}|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{2}==)$"];
}
@end
