...
{
    if (!idcardModel) {
        _name.rightTextField.textFiled.text  = @"";
        _six.rightTextField.textFiled.text = @"";
        _shenfenzheng.rightTextField.textFiled.text = @"";
        return;
    }
...
{
    NSString *msg = isSuccess ? @"" : @"上传失败";
    if ([imageKey isEqualToString:@"zukeSfzZheng"]) {
         if (!isSuccess){ ALERT(@"身份证正面上传失败");
             [self.param removeObjectForKey:@"zukeSfzZheng"];
             [_zhengmian setMaksLayerAlearMsg:msg];}
         else{[_zhengmian dismissMaskLayer];}
    }else if ([imageKey isEqualToString:@"zukeSfzFan"]){
        if (!isSuccess) {ALERT(@"身份证反面上传失败");
        [self.param removeObjectForKey:@"zukeSfzFan"];
            [_fanmian setMaksLayerAlearMsg:msg];}
        else{[_fanmian dismissMaskLayer];}
    }else if ([imageKey isEqualToString:@"handheldIdentityCard"]){
        if (!isSuccess) { ALERT(@"手持身份证上传失败");
        [self.param removeObjectForKey:@"handheldIdentityCard"];
            [_shouchizhao setMaksLayerAlearMsg:msg];}
        else{[_shouchizhao dismissMaskLayer];}
    }
}
...
- (BOOL)checkParaLegal
{
    ...
    if (![LWIdCardCheckTool validateCard:self.param[@"sfzNo"]]) {
        ALERT(@"身份证号格式不正确");
        return NO;
    }
